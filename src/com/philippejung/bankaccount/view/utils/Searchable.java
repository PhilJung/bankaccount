...
public abstract class Searchable<T> {
    ...
    private void select(int index, KeyEvent e) {
        if (index != -1) {
            ...
            updateText(getTypedText() + " (no match)");
        }
        if (index != -1) {
    ...
    public StringProperty searchingLabelProperty() {
        if (_searchingLabelProperty == null) {
            _searchingLabelProperty = new SimpleStringProperty("Searchable.searchFor");
        }
        ...
        public void handle(KeyEvent e) {
            ...
            if (e.getEventType() == KeyEvent.KEY_PRESSED) {
                if (isSelectAllKey(e)) {
                    ...
                    updateText(text + " " + MessageFormat.format("Searchable.found", count));
                    e.consume();
                    return;
                }
